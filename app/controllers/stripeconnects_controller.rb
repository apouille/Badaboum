...
def stripe_callback
            ...
            current_user.update!(uid: @resp.params["stripe_user_id"]) if @resp
            flash[:notice] = "Your account has been successfully created and is ready to process payments!"
   end
  

   def payment_profile
      @account = Stripe::Account.retrieve("#{current_user.uid.to_s}") if current_user.uid.present?
      @balance = Stripe::Balance.retrieve() if current_user.uid.present?
   end
